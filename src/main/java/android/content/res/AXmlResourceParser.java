...
public class AXmlResourceParser implements XmlResourceParser {
		...
		private final int find(int prefixOrUri, boolean prefix) {
			...
			for (int i = m_depth; i != 0; i--) {
				...
				for (; count != 0; count--) {
					...
					} else if (m_data[(offset + 1)] == prefixOrUri) {
						return m_data[offset];
					}
					offset -= 2;
				}
	...
	public int getAttributeListValue(String namespace, String attribute, String[] options, int i) {
		return 0;
	}
	...
	private final int findAttribute(String namespace, String attribute) {
		...
		int uri = namespace == null ? -1 : m_strings.find(namespace);
	...
	private final void doNext() throws IOException {
		...
		while (true) {
			...
			if (event == START_DOCUMENT) {
				...
				//System.out.println("chunkType[START_DOCUMENT]: " + Integer.toHexString(chunkType));
			} else {
